...
public class OrderController {
    ...
    public ResponseEntity<OrderResponse> placeOrder(@RequestBody final OrderRequest orderRequest, final UriComponentsBuilder uriComponentsBuilder) {
        final var orderResponse = orderService.placeOrder(orderRequest);
        final var uri = uriComponentsBuilder.path("/api/order/{id}").buildAndExpand(orderResponse.uuid()).toUri();
